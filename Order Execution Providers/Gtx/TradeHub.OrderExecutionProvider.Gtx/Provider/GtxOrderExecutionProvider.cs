...
namespace TradeHub.OrderExecutionProvider.Gtx.Provider
{
    public class GtxOrderExecutionProvider : QuickFix.MessageCracker, IApplication, IMarketOrderProvider, ILimitOrderProvider
    {
        private Type _type = typeof(GtxOrderExecutionProvider);
        ...
        #region GTX Order Client Members
        ...
        // Default Constructor
        public GtxOrderExecutionProvider()
        {
            // Set provider name
            _provider = Constants.OrderExecutionProvider.Gtx;

            _fixSettingsFile = AppDomain.CurrentDomain.BaseDirectory + @"\Config\" + "GtxFIXSettings.txt";
        }
        ...
        public bool Start()
        {
            try
            {
                if (this._initiator == null)
                {
                    ...
                    {
                        Logger.Info("GTX Fix Order Client Started.", _type.FullName, "Start");
                    }
                }
                else
                {
                    if (!this._initiator.IsStopped)
                    {
                        if (Logger.IsInfoEnabled)
                        {
                            Logger.Info("GTX Fix Order Client Already Started.", _type.FullName, "Start");
                        }
                    }
                    else
                    {
        ...
        public bool Stop()
        {
            try
            {
                if (!this._initiator.IsStopped)
                {
                    ...
                    if (Logger.IsInfoEnabled)
                    {
                        Logger.Info("GTX Fix Order Client Stoped.", _type.FullName, "Stop");
                    }
                }
                else
                {
                    if (Logger.IsInfoEnabled)
                    {
                        Logger.Info("GTX Fix Order Client Already Stoped.", _type.FullName, "Stop");
                    }
                }
                return true;
            }
        ...
        public void Crack(Message message, SessionID sessionId)
        {
            ...
            else if (message is QuickFix.FIX43.ExecutionReport)
            {
                OnMessage((QuickFix.FIX43.ExecutionReport)(message), sessionId);
            }
            else if (message is QuickFix.FIX43.OrderCancelReject)
            {
                OnMessage((QuickFix.FIX43.OrderCancelReject)(message), sessionId);
            }
        ...
        /// <param name="message"></param>
        /// <param name="sessionId"></param>
        private void OnMessage(QuickFix.FIX43.OrderCancelReject message, SessionID sessionId)
        {
            try
            {
        ...
        /// <summary>
        /// Creates a FIX4.3 OrderCancelRequest message for GTX
        /// </summary>
        /// <param name="order"></param>
        /// <returns></returns>
