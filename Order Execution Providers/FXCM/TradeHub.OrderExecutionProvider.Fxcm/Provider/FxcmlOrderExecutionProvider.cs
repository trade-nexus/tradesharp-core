...
namespace TradeHub.OrderExecutionProvider.Fxcm.Provider
{
    public class FxcmOrderExecutionProvider : QuickFix.MessageCracker, IApplication, IMarketOrderProvider, ILimitOrderProvider
    {
        private Type _type = typeof(FxcmOrderExecutionProvider);
        ...
        #region FXCM FIX Client Members

        private string _userName = string.Empty;
        private string _password = string.Empty;
        private string _tradeSenderCompId = string.Empty;
        private string _tradeTargetCompId = string.Empty;
        private string _tradeTargetSubId = string.Empty;
        private string _account = "00421753";
        ...
        public bool Start()
        {
            try
            {
                if (this._initiator == null)
                {
                    ...
                    {
                        Logger.Info("Fxcm Fix Order Client Started.", _type.FullName, "Start");
                    }
                }
                else
                {
                    if (!this._initiator.IsStopped)
                    {
                        if (Logger.IsInfoEnabled)
                        {
                            Logger.Info("Fxcm Fix Order Client Already Started.", _type.FullName, "Start");
                        }
                    }
                    else
                    {
        ...
        public bool Stop()
        {
            try
            {
                if (!this._initiator.IsStopped)
                {
                    ...
                    if (Logger.IsInfoEnabled)
                    {
                        Logger.Info("Fxcm Fix Order Client Stoped.", _type.FullName, "Stop");
                    }
                }
                else
                {
                    if (Logger.IsInfoEnabled)
                    {
                        Logger.Info("Fxcm Fix Order Client Already Stoped.", _type.FullName, "Stop");
                    }
                }
                return true;
            }
        ...
        private void OnMessage(QuickFix.FIX44.ExecutionReport executionReport, SessionID sessionId)
        {
            try
            {
                char orderStatus = executionReport.OrdStatus.getValue();
                if (orderStatus == OrdStatus.STOPPED)
                    return;
        ...
        private Execution PopulateExecution(QuickFix.FIX44.ExecutionReport executionReport)
        {
                                      ...
                                      _provider, executionReport.ClOrdID.getValue())
            {
                ...
                ExecutionPrice = Convert.ToDecimal(executionReport.LastPx.getValue(), CultureInfo.InvariantCulture),
            ...
            Order order = new Order(_provider)
            {
                ...
                BrokerOrderID = executionReport.OrderID.getValue(),
                OrderSide = orderSide,
        ...
        private void PopulateFixSettings()
        {
            try
            {
                ...
                if (settings != null && settings.Count > 0)
                {
                    ...
                    settings.TryGetValue("TargetSubID", out _tradeTargetSubId);
                }
            }
            catch (Exception exception)
            {
