...
namespace TradeHub.OrderExecutionProvider.Integral.Provider
{
    public class IntegralOrderExecutionProvider : QuickFix.MessageCracker, IApplication, IMarketOrderProvider, ILimitOrderProvider
    {
        private Type _type = typeof(IntegralOrderExecutionProvider);
        ...
        #region Integral FIX Client Members
        ...
        // Default Constructor
        public IntegralOrderExecutionProvider()
        {
            // Set provider name
            _provider = Constants.OrderExecutionProvider.Integral;
        ...
        public bool Start()
        {
            try
            {
                if (this._initiator == null)
                {
                    ...
                    {
                        Logger.Info("Integral Fix Order Client Started.", _type.FullName, "Start");
                    }
                }
                else
                {
                    if (!this._initiator.IsStopped)
                    {
                        if (Logger.IsInfoEnabled)
                        {
                            Logger.Info("Integral Fix Order Client Already Started.", _type.FullName, "Start");
                        }
                    }
                    else
                    {
        ...
        public bool Stop()
        {
            try
            {
                if (!this._initiator.IsStopped)
                {
                    ...
                    if (Logger.IsInfoEnabled)
                    {
                        Logger.Info("Integral Fix Order Client Stoped.", _type.FullName, "Stop");
                    }
                }
                else
                {
                    if (Logger.IsInfoEnabled)
                    {
                        Logger.Info("Integral Fix Order Client Already Stoped.", _type.FullName, "Stop");
                    }
                }
                return true;
            }
