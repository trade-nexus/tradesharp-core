...
namespace AskBasedLongShortStrategy
{
    /// <summary>
    /// Strategy
    /// </summary>
    [TradeHubAttributes("Ask based long short strategy", typeof(Strategy))]
    public class Strategy : TradeHubStrategy
    {
        /// <summary>
        /// Symbol to use for strategy execution
        /// </summary>
        ...
        {
            Console.WriteLine("########################################CW->On Tick Arrived.");

            if (_oldAskPrice > 0 && tick.AskPrice < _oldAskPrice)
            {
                Console.WriteLine("#######################################CW->Sending Buy Order.");
                SendMarketOrder("M1", new Security() { Symbol = _symbol }, 100, OrderSide.BUY);
            }
            else
            {
                Console.WriteLine("#######################################CW->Sending Buy Order.");
                SendMarketOrder("M1", new Security() { Symbol = _symbol }, 100, OrderSide.SELL);
            }

            _oldAskPrice = tick.AskPrice;
        ...
        {
            Console.WriteLine("########################################CW->On Bar Arrived.");
        }
