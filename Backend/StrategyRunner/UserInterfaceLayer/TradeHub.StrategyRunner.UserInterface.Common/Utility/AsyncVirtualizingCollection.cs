...
namespace TradeHub.StrategyRunner.UserInterface.Common.Utility
{
    /// <summary>
    ...
    public class AsyncVirtualizingCollection<T> : VirtualizingCollection<T>, INotifyCollectionChanged, INotifyPropertyChanged
    {
        ...
        {
            base.ItemsProvider.AddItem(item);
            LoadCountWork(0);
            LoadPage(0);
            //IList<T> page = FetchPage(0);
            //SynchronizationContext.Send(LoadPageCompleted, new object[] { 0, page });
