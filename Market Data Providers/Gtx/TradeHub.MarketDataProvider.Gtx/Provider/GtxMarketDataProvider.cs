...
namespace TradeHub.MarketDataProvider.Gtx.Provider
{
    public class GtxMarketDataProvider : MessageCracker, IApplication, ILiveTickDataProvider
    {
        private Type _type = typeof(GtxMarketDataProvider);

        private string _provider = Common.Core.Constants.MarketDataProvider.Gtx;
        ...
        public bool Start()
        {
            try
            {
                if (this._initiator == null)
                {
                    ...
                    {
                        Logger.Info("GTX Data Client Started.", _type.FullName, "Start");
                    }
                }
                else
                {
                    if (!this._initiator.IsStopped)
                    {
                        if (Logger.IsInfoEnabled)
                        {
                            Logger.Info("GTX Data Client Already Started.", _type.FullName, "Start");
                        }
                    }
                    else
                    {
        ...
        public bool Stop()
        {
            try
            {
                if (!this._initiator.IsStopped)
                {
                    ...
                    if (Logger.IsInfoEnabled)
                    {
                        Logger.Info("GTX Data Client Stoped.", _type.FullName, "Stop");
                    }
                }
                else
                {
                    if (Logger.IsInfoEnabled)
                    {
                        Logger.Info("GTX Data Client Already Stoped.", _type.FullName, "Stop");
                    }
                }
                return true;
            }
